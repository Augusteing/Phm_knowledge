...
def check_prompt_file():
    ...
    papers_dir = EXP_DIR / "data" / "raw" / "papers"
    priority_dir = papers_dir / "priority"
    ...
    # 若未发现通用 prompt，检查是否存在 per-article 提示词（prompt_*.txt）
    if prompt_dir.exists():
        per_article = list(prompt_dir.glob("prompt_*.txt"))
        if per_article:
            # 统计与 priority 下论文的匹配情况（同名去扩展名）
            matched = 0
            total_priority = 0
            if priority_dir.exists():
                priority_md = [p.stem for p in priority_dir.glob("*.md")]
                total_priority = len(priority_md)
                names = {p.stem for p in per_article}
                # prompt_前缀去除
                normalized = {n[7:] for n in names if n.startswith("prompt_")}
                matched = sum(1 for stem in priority_md if stem in normalized)
            print(f"✓ 检测到 per-article 提示词 {len(per_article)} 个（与优先论文匹配 {matched}/{total_priority}）。")
            if general_dir.exists():
                general_cnt = len(list(general_dir.glob("*.md")))
                if general_cnt > 0:
                    print("ℹ️  提示：若后续处理 general 论文且未提供对应 per-article 提示词，将回退到 EXTRACT_PROMPT_FILE 或通用 prompt；如均未设置会报错。")
            return True
    # fallback: 唯一 .txt
    if prompt_dir.exists():
        ...
        if len(txts) == 1:
            ...
            return True
    print(f"⚠️  未找到通用 prompt 文件。实验三中 priority 论文将优先使用 per-article 提示词（形如 prompt_<文件名>.txt）。\n   如需默认回退，请在 {prompt_dir} 放置 prompt实验二.txt / prompt_exp2.txt / prompt.txt，或设置 EXTRACT_PROMPT_FILE。")
    return False
