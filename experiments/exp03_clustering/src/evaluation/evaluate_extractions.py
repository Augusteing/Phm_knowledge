...
def normalize_extraction_data(data: any) -> dict:
    """将抽取结果归一化为 {entities:[], relations:[]} 的字典结构。
    兼容以下输入形态：
    - dict 且包含 entities/relations（直接返回）
    - list 且长度为 1，元素为 dict（取第一个元素）
    - list[dict] 且元素中包含 entities/relations（合并所有元素的 entities/relations）
    - list[entity-like]（仅有实体列表）：包装为 {entities: 列表, relations: []}
    其他形态将抛出 ValueError。
    """
    # dict: 正常情况
    if isinstance(data, dict):
        ...
        for key in ("data", "result", "output"):
            inner = data.get(key)
            if isinstance(inner, dict) and ("entities" in inner or "relations" in inner):
                inner.setdefault("entities", [])
                inner.setdefault("relations", [])
                return inner
        raise ValueError("抽取 JSON 为对象，但缺少 entities/relations 字段。")
    ...
    if isinstance(data, list):
        ...
        if len(data) == 1 and isinstance(data[0], dict):
            return normalize_extraction_data(data[0])
        # 列表元素均为 dict，且其中包含 entities/relations：尝试合并
        if all(isinstance(x, dict) for x in data):
            merged_entities = []
            merged_relations = []
            any_valid = False
            for x in data:
                ents = x.get("entities") if isinstance(x.get("entities"), list) else []
                rels = x.get("relations") if isinstance(x.get("relations"), list) else []
                if ents or rels:
                    any_valid = True
                merged_entities.extend(ents)
                merged_relations.extend(rels)
            if any_valid:
                return {"entities": merged_entities, "relations": merged_relations}
        # 列表元素像是实体项：尝试包装
        if all(isinstance(x, dict) for x in data) and any("name" in x or "entity" in x for x in data):
            return {"entities": data, "relations": []}
        raise ValueError("抽取 JSON 为数组，无法判定其结构用于评估。")

    raise ValueError(f"不支持的抽取 JSON 类型: {type(data).__name__}")
...
"""
    ...
    try:
        ...
        # 兼容返回值为 list 的情况（常见为单元素包裹）
        if isinstance(evaluated_data, list):
            if len(evaluated_data) == 1 and isinstance(evaluated_data[0], dict):
                evaluated_data = evaluated_data[0]
            else:
                raise ValueError(f"返回的 JSON 类型为数组且无法解包（长度={len(evaluated_data)}）。")
        # 验证返回格式
        if not isinstance(evaluated_data, dict) or ("entities" not in evaluated_data or "relations" not in evaluated_data):
            raise ValueError(f"返回的 JSON 非对象或缺少必需字段（entities/relations）。实际类型={type(evaluated_data).__name__}")
...
def evaluate_model_results(client: OpenAI, eval_prompt_template: str, model_name: str, extraction_dir: Path, eval_output_root: Path, eval_log_dir: Path, overwrite: bool = False):
    ...
    for json_file in tqdm(json_files, desc=f"评估 {model_name}", unit="篇"):
        ...
        try:
            ...
            # 归一化抽取 JSON（容忍顶层数组/嵌套）
            try:
                extraction_data = normalize_extraction_data(raw_data)
            except Exception as norm_err:
                tqdm.write(f"   ⚠️ 跳过（抽取JSON结构不规范）: {paper_name} -> {norm_err}")
                log_entry = {
                    "time": now_iso(),
                    "paper": paper_name,
                    "model": model_name,
                    "status": "skipped",
                    "reason": f"invalid_extraction_json: {norm_err}",
                }
                eval_log.append(log_entry)
                failed_count += 1
                continue
