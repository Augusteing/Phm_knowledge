...
def _resolve_prompt_file() -> str:
    """解析 Prompt 文件路径，支持环境变量与多种命名回退。

    优先顺序：
    1) 环境变量 EXTRACT_PROMPT_FILE 指定的绝对/相对路径
    2) config/prompt 目录下按优先名尝试：
       - prompt实验二.txt / prompt_exp2.txt / prompt.txt / prompt实验一.txt
    3) 若目录内存在唯一的 .txt 文件，则使用该文件
    否则抛出错误。
    """
    ...
    raise FileNotFoundError(
        f"未找到 prompt 文件，请在 {PROMPT_DIR} 中放置 prompt实验二.txt 或通过 EXTRACT_PROMPT_FILE 指定"
    )
...
DEEPSEEK_DIR = os.path.join(EXTRACTIONS_DIR, "deepseek")      # DeepSeek结果存放目录
IN_SCOPE_DIR = os.path.join(DEEPSEEK_DIR, "priority")         # 优先论文输出
OUT_SCOPE_DIR = os.path.join(DEEPSEEK_DIR, "general")         # 普通论文输出
LOG_DIR = os.path.join(OUTPUTS_DIR, "logs", "deepseek")       # 日志文件存放目录
...
# 初始化日志记录器
logger = ExtractionLogger(LOG_DIR, "deepseek")
...
# ------------------------------
# 加载 prompt 与 schema（实验二引入 schema）
# ------------------------------
with open(PROMPT_FILE, "r", encoding="utf-8") as f:
    prompt_template = f.read()

# schema 读取与过滤：默认仅注入 entity_types 与 relation_types，避免提示词过长
...
# ------------------------------
# 批次询问机制
# - FIRST_BATCH_SIZE: 第一批处理数量（默认 10），完成后询问是否继续
# - IN_SCOPE_LIMIT: 优先论文总数限制（默认 50）
# - AUTO_CONTINUE_REST: 自动继续处理剩余论文（y/n）
# ------------------------------
...
# 分批策略：
# 第1批：前 FIRST_BATCH_SIZE 篇（询问后继续）
# 第2批：FIRST_BATCH_SIZE 到 IN_SCOPE_LIMIT 篇
# 第3批：IN_SCOPE_LIMIT 之后的所有论文
...
def _process_one(paper_file: str, target_dir: str, progress_bar=None):
    ...
    # 填充 prompt（占位符替换；实验二注入 schema，如无占位符则追加在末尾）
    prompt_filled = (
        prompt_template
    ...
    # 针对 DeepSeek 附加 JSON 输出提示
    prompt_filled = prompt_filled + build_json_hint()
    ...
    for attempt in range(max_retries):
        ...
        try:
            ...
            content = response.choices[0].message.content if response and response.choices and response.choices[0].message else ""
            ...
            logger.add_log_entry(
                ...
                attempts=attempts,
                max_tokens_used=curr_max_tokens,
...
if first_batch:
    ...
    progress_bar_1 = _iter_with_progress(first_batch, desc="DeepSeek [1/3] 试运行")
...
if not aborted_for_balance and second_batch and (AUTO_CONTINUE_REST in {"y", "yes"} or proceed):
    ...
    progress_bar_2 = _iter_with_progress(second_batch, desc="DeepSeek [2/3] 优先论文")
...
if not aborted_for_balance and third_batch and (AUTO_CONTINUE_REST in {"y", "yes"} or proceed):
    ...
    progress_bar_3 = _iter_with_progress(third_batch, desc="DeepSeek [3/3] 普通论文")
